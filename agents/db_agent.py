...
async def db_agent_stream(query: str, user_id: str, query_id: str, file_id_list : list, notes : str,timeout_: int = 55):
    try:
        ...
        async with AsyncConnectionPool(
            ...
            prompt = f"""
        Your task is to figure out a way to find that you have to find
        the SQL query to get the value for the given query by the user
        ...
        - You have the following sheets to select from for `table_name`
            > `balance sheet`, `cash flow statement`, `profit and loss`

        - When a user asks a query deduce that whether you will have to retrive it or calculate it, follow these steps:
            1. Deduce which is the table this metric could be possibly located in
            2. Then filter by year if the year is sepcified
            3. Do not filter in the particulars column as you will end up not matching the exact name of the metric
            4. If you find the metric return the metric as it is or else you might have to calculate it
            5. Then repeat this cycle to find the metrics and calculate it.
...
- Ensure the final output strictly reflects the requested year’s values only.
                ...
                async for msg, metadata in langgraph_agent_executor.astream({"messages": [("system", prompt),("system", query_id_prompt), ("human", query), ("system", common_prompt)]}, config, stream_mode="messages"):
